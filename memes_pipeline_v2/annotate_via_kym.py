...
def precompute_vectors(hashes_dic, phases_path):
    ...
    if os.path.isfile(pickle_file):
        ...
        print('[w] fetched precomputed vectors from ',
        ...
        print('[w] generated vectors and stored them in ',
              pickle_file, 'new processed', len(hashes))
...
def seek_queue_many(ids_i, ids_j, hashes_i, hashes_j, outdir, blacklist, hashes_diff):
    ...
    # shape=[None, 64] [len_hashes]
    queue_hashes_j = tf.compat.v1.placeholder(tf.bool, shape=[batch_size, None])

    queue = tf.queue.FIFOQueue(capacity=50, dtypes=[
    ...
    enqueue_threads = [threading.Thread(target=check_batch_many, args=[sess, hashes_i, enqueue_op, init_i,
    ...
    for _ in range(total_tasks):
        ...
        diff, filter, where = sess.run([diff_op_many, filter_op, where_op], feed_dict={
                                       diff_hash_i: hash_i, diff_hashes_j: hashes_j})
        for j in where:
            j_pos = j[0]
...
def main(options, arguments):
    ...
    ''' 
        Process KYM 
    '''
    kym_phashes_by_meme_dic, kym_images_dic, kym_images_dic_reverse, kym_meme_name = process_kym_files(
        kym_phashes_file)
    ...
    with tf.device(device):
        seek_queue_many(list(clusters.keys()), list(kym_images_dic_reverse.keys(
        )), hashes_i, hashes_j, outfile, blacklist, hashes_diff)
...
if __name__ == "__main__":
    ...
    parser.add_option("-p", "--phashes", dest='phashes',
                      default='phashes.txt', help="phashes file")
    parser.add_option("-c", "--clustering", dest='clustering',
                      default='clustering_output.txt', help="clustering output file")
    parser.add_option("-o", "--output", dest='output',
                      default='clustering_selection.json', help="output file with kym annotations")
    parser.add_option("-k", "--distance", dest='distance',
                      default=8, help="threshold for selecting annotations")
