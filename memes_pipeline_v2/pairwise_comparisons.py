...
tf.compat.v1.app.flags.DEFINE_integer("batch_size", 4000000, "Search batch size")
...
config=tf.compat.v1.ConfigProto(allow_soft_placement=True, log_device_placement=True) #allow_soft_placement=True, log_device_placement=True
...
def seek_queue_many(ids, hashes, outdir, blacklist, hashes_diff):
    ...
    queue_hash_i = tf.compat.v1.placeholder(tf.bool, shape=[None, 64])
    queue_hashes_j = tf.compat.v1.placeholder(tf.bool, shape=[batch_size, None]) #shape=[None, 64] [len_hashes]
...
def seek_queue_many_device(ids, hashes, outdir, blacklist, hashes_diff, devices, device):
    ...
    queue = tf.queue.FIFOQueue(capacity=100, dtypes=[tf.int32, tf.bool], shapes=[[], [64]])
...
def convert_vectors(hashes):
    thashes = []
    for h in hashes:
        thashes.append(tf.convert_to_tensor(value=h, dtype=tf.int64))
    return thashes
